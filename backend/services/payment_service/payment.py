...
def create_signed_receipt_mock(transaction_data: dict) -> dict:
    jws = f"HEADER.PAYLOAD_{secrets.token_hex(8)}.SIGNATURE_{secrets.token_hex(16)}"
    return {"signed_receipt": jws, "signed_by": "MockHSM-KeyID-12345"}
...
async def checkout_cart(request: Request):
    ...
    order_id = f"CART-{secrets.token_hex(4).upper()}"
...
async def create_payment(request: Request,
    ...
    try:
        ...
        if intent.status == "succeeded":
            ...
            signed_receipt = create_signed_receipt_mock(receipt_data)
            ...
            return templates.TemplateResponse("error.html", {"request": request, "error": f"Payment requires confirmation. Status: {intent.status}"})
        ...
        return templates.TemplateResponse("error.html", {"request": request, "error": "Error processing payment. Please try again."})